...
exports.resetNetworksToDefault = exports.assertArbitrumNetworkHasTokenBridge = exports.mapL2NetworkToArbitrumNetwork = exports.mapL2NetworkTokenBridgeToTokenBridge = exports.getMulticallAddress = exports.getNitroGenesisBlock = exports.registerCustomArbitrumNetwork = exports.getArbitrumNetworkInformationFromRollup = exports.getArbitrumNetworks = exports.getArbitrumNetwork = exports.getChildrenForNetwork = exports.isParentNetwork = void 0;
/*
 ...
 */
const isParentNetwork = (parentChainOrChainId) => {
    const parentChainId = typeof parentChainOrChainId === 'number'
...
exports.isParentNetwork = isParentNetwork;
/**
 * Returns a list of children chains for the given chain or chain id.
 */
const getChildrenForNetwork = (parentChainOrChainId) => {
    const parentChainId = typeof parentChainOrChainId === 'number'
        ? parentChainOrChainId
        : parentChainOrChainId.chainId;
    return getArbitrumNetworks().filter(arbitrumChain => arbitrumChain.parentChainId === parentChainId);
};
exports.getChildrenForNetwork = getChildrenForNetwork;
function getArbitrumNetwork(signerOrProviderOrChainId) {
...
exports.getArbitrumNetwork = getArbitrumNetwork;
function getArbitrumNetworkByChainId(chainId) {
...
async function getArbitrumNetworkBySignerOrProvider(signerOrProvider) {
    const provider = signerOrProvider_1.SignerProviderUtils.getProviderOrThrow(signerOrProvider);
 ...
 */
function getArbitrumNetworks() {
    return Object.values(networks);
}
exports.getArbitrumNetworks = getArbitrumNetworks;
/**
 ...
 */
async function getArbitrumNetworkInformationFromRollup(rollupAddress, parentProvider) {
    const rollup = RollupAdminLogic__factory_1.RollupAdminLogic__factory.connect(rollupAddress, parentProvider);
    const [bridge, inbox, sequencerInbox, outbox, confirmPeriodBlocks] = await Promise.all([
        rollup.bridge(),
        rollup.inbox(),
...
exports.getArbitrumNetworkInformationFromRollup = getArbitrumNetworkInformationFromRollup;
/**
 ...
 */
function registerCustomArbitrumNetwork(network, options) {
    var _a;
    const throwIfAlreadyRegistered = (_a = options === null || options === void 0 ? void 0 : options.throwIfAlreadyRegistered) !== null && _a !== void 0 ? _a : false;
    if (!network.isCustom) {
        throw new errors_1.ArbSdkError(`Custom network ${network.chainId} must have isCustom flag set to true`);
    }
    if (typeof networks[network.chainId] !== 'undefined') {
        ...
        if (throwIfAlreadyRegistered) {
            throw new errors_1.ArbSdkError(message);
        }
...
function getNitroGenesisBlock(arbitrumChainOrChainId) {
    const arbitrumChainId = typeof arbitrumChainOrChainId === 'number'
    ...
    if (arbitrumChainId === 42161) {
        return constants_1.ARB1_NITRO_GENESIS_L2_BLOCK;
    }
    return 0;
}
exports.getNitroGenesisBlock = getNitroGenesisBlock;
async function getMulticallAddress(providerOrChainId) {
    const chains = getArbitrumNetworks();
    const chainId = typeof providerOrChainId === 'number'
        ? providerOrChainId
    ...
    if (typeof childChain === 'undefined') {
        throw new Error(`Failed to retrieve Multicall address for chain: ${chainId}`);
    }
 ...
 */
function mapL2NetworkTokenBridgeToTokenBridge(input) {
    return {
...
exports.mapL2NetworkTokenBridgeToTokenBridge = mapL2NetworkTokenBridgeToTokenBridge;
/**
 * Maps the old {@link L2Network} (from SDK v3) to {@link ArbitrumNetwork} (from SDK v4).
 */
function mapL2NetworkToArbitrumNetwork(l2Network) {
    return Object.assign(Object.assign({}, l2Network), { 
        // Map properties that were changed
        chainId: l2Network.chainID, parentChainId: l2Network.partnerChainID, tokenBridge: mapL2NetworkTokenBridgeToTokenBridge(l2Network.tokenBridge) });
}
exports.mapL2NetworkToArbitrumNetwork = mapL2NetworkToArbitrumNetwork;
/**
 ...
 */
function assertArbitrumNetworkHasTokenBridge(network) {
    if (typeof network === 'undefined' ||
        !('tokenBridge' in network) ||
        typeof network.tokenBridge === 'undefined') {
        throw new errors_1.ArbSdkError(`The ArbitrumNetwork object with chainId ${network.chainId} is missing the token bridge contracts addresses. Please add them in the "tokenBridge" property.`);
    }
}
exports.assertArbitrumNetworkHasTokenBridge = assertArbitrumNetworkHasTokenBridge;
